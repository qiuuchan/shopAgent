# -*- coding: utf-8 -*-
"""
engine.alert_dedup —— 告警去重与防抖（TIK-005 连接断开告警链路）
================================================================
本文件用途：为「拼多多长连接断开 / Cookie 刷新失败」等系统事件提供**去重与防抖**
能力，避免重连风暴在短时内把企微 Webhook 刷爆（需求 26 / 关联工单 TIK-005）。
...
- **静默期（silence_seconds）**：默认 30 分钟（1800 秒，规范 17 全链路北京时间口径
  ...
  后自动恢复。
- **resolve 立即恢复**：连接恢复 / 事件人工确认后调用 ``resolve`` 清除该键的静默
  状态，使下一次同类事件可立即再次告警（例如恢复后再次断开也应能重新通知）。
- **零外部依赖**：纯内存结构（datetime 比对），无 I/O，便于单元与属性测试；
  时间统一经 ``common.utils.time_utils.now_beijing``（北京时间，规范 17）。
- **线程 / 协程安全**：本服务为单进程 asyncio 事件循环模型，去重表仅在事件循环内
  访问，无需额外加锁（保持简单，符合「不做过度设计」原则）。

企微 Webhook 地址当前**未提供**，因此本模块只负责「去重决策 + 尽力而为通知」，
真实发送回调（send_cb）由调用方注入；未注入时仅打 info 日志（禁用 debug，规范 38）。
...
logger = logging.getLogger("engine.alert_dedup")
